...
<iframe width="560" height="380" src="https://www.youtube.com/embed/HMTtpsVX2mo" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

## Installation

Once you have [Filament Admin Panel](https://filamentphp.com/docs/2.x/admin/installation) configured. You can install this package via composer:
```bash
...
Apart from these this plugin also adds the following [Filament's Render Hooks](https://filamentphp.com/docs/2.x/admin/appearance#render-hooks) to the base layout:
...
public static function getBlockSchema(): Block
```
You may use any [Fields](https://filamentphp.com/docs/2.x/forms/fields) to make up your schema.
...
Underneath the hood `PageBuilder` is just a Filament's [Builder](https://filamentphp.com/docs/2.x/forms/fields#builder) field. Like other filament fields this field also has methods that can be used to modify it. You may configure it like this:
```php
...
```
> If you are using [Shield](https://filamentphp.com/plugins/shield) just register the `PagePolicy` generated by it
